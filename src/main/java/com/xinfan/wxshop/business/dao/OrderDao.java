...
public class OrderDao extends SqlSessionDaoSupport {
	...
	public Pagination getCustomerCenterOrderPageList(Map param, Pagination page) {
		...
		List<Order> list = getSqlSession().selectList(wrapCommand("getCustomerCenterOrderPageList"), param, page);
	...
	public int getUnPayOrderCount(int customerId, int state) {

		OrderExample example = new OrderExample();
		example.createCriteria().andCustomerIdEqualTo(customerId).andStatusEqualTo(state);

		return getSqlSession().selectOne(wrapCommand("countByExample"), example);
	}
